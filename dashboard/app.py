...
def build_backtest_chart(results: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        ...
        template="plotly_white",
        margin=dict(l=20, r=20, t=40, b=20),
        title="Recent Backtest: Actual Close vs Predicted Range",
        hovermode="x unified",
        legend=dict(orientation="h", y=1.02, x=1, xanchor="right", yanchor="bottom"),
    )
...
def main() -> None:
    ...
    with st.expander("Model insights", expanded=True):
        st.markdown(
            "- Prediction intervals widen when recent BTC volatility rises.\n"
            "- Rolling volatility lets the model adapt to current market conditions instead of using one static risk estimate.\n"
            "- Student-t shocks give the GBM simulation fatter tails, helping account for extreme BTC moves."
        )
    st.plotly_chart(build_backtest_chart(backtest_results), use_container_width=True)
